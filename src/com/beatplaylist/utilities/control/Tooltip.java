...
import com.beatplaylist.utilities.control.skin.TooltipSkin;
...
public class Tooltip extends PopupControl {
	...
	/**
	 * Associates the given {@link Tooltip} with the given {@link Node}. The tooltip can then behave similar to when it is set on any {@link Control}. A single tooltip can be associated with multiple nodes.
	 * 
	 * @param node
	 *            the node
	 * @param t
	...
	/**
	 * Specifies the behavior for lines of text <em>when text is multiline</em>. Unlike {@link #contentDisplayProperty() contentDisplay} which affects the graphic and text, this setting only affects multiple lines of text relative to the text bounds.
	 * 
	...
	/**
	 * Specifies the behavior to use if the text of the {@code Tooltip} exceeds the available space for rendering the text.
	 * 
	...
	private final ObjectProperty<Duration> showDelayProperty = new SimpleStyleableObjectProperty<>(SHOW_DELAY, this, "showDelay", new Duration(100));
	...
	private final ObjectProperty<Duration> showDurationProperty = new SimpleStyleableObjectProperty<>(SHOW_DURATION, this, "showDuration", new Duration(Integer.MAX_VALUE));
	...
	private final ObjectProperty<Duration> hideDelayProperty = new SimpleStyleableObjectProperty<>(HIDE_DELAY, this, "hideDelay", new Duration(100));
	...
	private static final CssMetaData<Tooltip.CSSBridge, Font> FONT = new FontCssMetaData<Tooltip.CSSBridge>("-fx-font", Font.getDefault()) {
	...
	private static final CssMetaData<Tooltip.CSSBridge, TextAlignment> TEXT_ALIGNMENT = new CssMetaData<Tooltip.CSSBridge, TextAlignment>("-fx-text-alignment", new EnumConverter<TextAlignment>(TextAlignment.class), TextAlignment.LEFT) {
	...
	private static final CssMetaData<Tooltip.CSSBridge, OverrunStyle> TEXT_OVERRUN = new CssMetaData<Tooltip.CSSBridge, OverrunStyle>("-fx-text-overrun", new EnumConverter<OverrunStyle>(OverrunStyle.class), OverrunStyle.ELLIPSIS) {
	...
	private static final CssMetaData<Tooltip.CSSBridge, Boolean> WRAP_TEXT = new CssMetaData<Tooltip.CSSBridge, Boolean>("-fx-wrap-text", BooleanConverter.getInstance(), Boolean.FALSE) {
	...
	private static final CssMetaData<Tooltip.CSSBridge, String> GRAPHIC = new CssMetaData<Tooltip.CSSBridge, String>("-fx-graphic", StringConverter.getInstance()) {
	...
	private static final CssMetaData<Tooltip.CSSBridge, ContentDisplay> CONTENT_DISPLAY = new CssMetaData<Tooltip.CSSBridge, ContentDisplay>("-fx-content-display", new EnumConverter<ContentDisplay>(ContentDisplay.class), ContentDisplay.LEFT) {
	...
	private static final CssMetaData<Tooltip.CSSBridge, Number> GRAPHIC_TEXT_GAP = new CssMetaData<Tooltip.CSSBridge, Number>("-fx-graphic-text-gap", SizeConverter.getInstance(), 4.0) {
	...
	private static final CssMetaData<Tooltip.CSSBridge, Duration> SHOW_DELAY = new CssMetaData<Tooltip.CSSBridge, Duration>("-fx-show-delay", DurationConverter.getInstance(), new Duration(1000)) {
	...
	private static final CssMetaData<Tooltip.CSSBridge, Duration> SHOW_DURATION = new CssMetaData<Tooltip.CSSBridge, Duration>("-fx-show-duration", DurationConverter.getInstance(), new Duration(5000)) {
	...
	private static final CssMetaData<Tooltip.CSSBridge, Duration> HIDE_DELAY = new CssMetaData<Tooltip.CSSBridge, Duration>("-fx-hide-delay", DurationConverter.getInstance(), new Duration(200)) {
	...
	static {
		final List<CssMetaData<? extends Styleable, ?>> styleables = new ArrayList<CssMetaData<? extends Styleable, ?>>(PopupControl.getClassCssMetaData());
		styleables.add(FONT);
	...
	private static class TooltipBehavior {
		/*
		 * There are two key concepts with Tooltip: activated and visible. A Tooltip is activated as soon as a mouse move occurs over the target node. When it becomes activated, we start off the ACTIVATION_TIMER. If the ACTIVATION_TIMER expires before another mouse event occurs, then we will show the popup. This timer typically lasts about 1 second.
		 *
		 * Once visible, we reset the ACTIVATION_TIMER and start the HIDE_TIMER. This second timer will allow the tooltip to remain visible for some time period (such as 5 seconds). If the mouse hasn't moved, and the HIDE_TIMER expires, then the tooltip is hidden and the tooltip is no longer activated.
		 *
		 * If another mouse move occurs, the ACTIVATION_TIMER starts again, and the same rules apply as above.
		 *
		 * If a mouse exit event occurs while the HIDE_TIMER is ticking, we reset the HIDE_TIMER. Thus, the tooltip disappears after 5 seconds from the last mouse move.
		 *
		 * If some other mouse event occurs while the HIDE_TIMER is running, other than mouse move or mouse enter/exit (such as a click), then the tooltip is hidden, the HIDE_TIMER stopped, and activated set to false.
		 *
		 * If a mouse exit occurs while the HIDE_TIMER is running, we stop the HIDE_TIMER and start the LEFT_TIMER, and immediately hide the tooltip. This timer is very short, maybe about a 1/2 second. If the mouse enters a new node which also has a tooltip before LEFT_TIMER expires, then the second tooltip is activated and shown immediately (the ACTIVATION_TIMER having been bypassed), and the HIDE_TIMER is started. If the LEFT_TIMER expires and there is no mouse movement over a control with a tooltip, then we are back to the initial steady state where the next mouse move over a node with a tooltip installed will start the ACTIVATION_TIMER.
		 */
