...
public class Generate2FACode {
	...
	public static void send(Button twoFactorButton) {
		Post post = new Post();
		post.setDetails();
		post.setPacketType(PacketType.GENERATE_2FA_CODE);
			...
			public void onSuccess(Post post) {
				Platform.runLater(() -> {
					...
					if (response.getJSONString("response").equals("success")) {
						String image_url = response.getJSONString("qr_url");
						Popup.display2FAPopup(image_url, twoFactorButton);
					}
				});
			}
